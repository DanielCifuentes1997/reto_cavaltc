      ...
      >
        <div className="w-full max-w-lg">
          <div className="text-center mb-8">
            ...
            <h1 className="text-2xl font-extrabold text-white">Registra tu empresa</h1>
          ...
          >
            <form onSubmit={handleOnboardingSubmit} className="flex flex-col gap-5">
              ...
              <div className="flex flex-col gap-1.5">
                <label className="text-xs font-semibold text-slate-300 uppercase tracking-wide">NIT *</label>
                <input
                  type="text"
                  value={form.nit}
              ...
              <div className="flex flex-col gap-1.5">
                <label className="text-xs font-semibold text-slate-300 uppercase tracking-wide">Sector *</label>
                <select
                  value={form.sector}
              ...
              {formError && (
                <p className="text-red-400 text-sm bg-red-400/10 rounded-lg px-4 py-2">{formError}</p>
              )}
  ...
  return (
    <div className="min-h-screen flex flex-col bg-cavaltec-light">
        ...
        <div className="container mx-auto px-4 h-20 flex items-center justify-between gap-4">
            ...
            <div className="w-8 h-8 rounded-lg flex items-center justify-center"
              ...
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none">
                <path d="M12 2L4 5.5v6c0 4.97 3.47 9.63 8 10.73C16.53 21.13 20 16.47 20 11.5v-6L12 2z"
                  stroke="#f0b429" strokeWidth="1.5" strokeLinejoin="round" fill="rgba(240,180,41,0.1)" />
              </svg>
            </div>
            <div>
              <img src="/logo_blanco.png" alt="CAVALTEC" className="h-11 w-auto object-contain" />
              {companyName && <p className="text-slate-400 text-xs leading-none mt-0.5">{companyName}</p>}
            </div>
          </div>
          ...
          <div className="hidden sm:flex items-center gap-2 bg-white/5 rounded-xl px-4 py-2 border border-white/10">
            <div className="w-2 h-2 rounded-full"
              style={{ background: score >= 80 ? "#22c55e" : score >= 50 ? "#f0b429" : "#ef4444" }} />
      ...
      <main className="container mx-auto px-4 py-6 flex flex-col gap-6 flex-grow">
        <section className="grid grid-cols-1 lg:grid-cols-[340px_1fr] gap-5 items-start">
          <div className="flex flex-col gap-4 lg:sticky lg:top-24 lg:max-h-[calc(100vh-6rem)] lg:overflow-y-auto">
            <div className="bg-cavaltec-dark rounded-2xl shadow-sm border border-slate-800 p-4">
              <ChestViewer />
            </div>
            <div className="bg-white rounded-2xl shadow-sm border border-slate-200 flex flex-col items-center justify-center py-5 px-4">
              <ComplianceGauge score={score} />
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-4">
                <p className="text-xs text-slate-400 font-semibold uppercase tracking-wide mb-1">Riesgo</p>
                <p className="text-xl font-extrabold"
                  style={{ color: score >= 80 ? "#22c55e" : score >= 50 ? "#f0b429" : "#ef4444" }}>
                  {score >= 80 ? "Bajo" : score >= 50 ? "Medio" : "Alto"}
                </p>
                <p className="text-xs text-slate-400 mt-1 leading-snug">
                  {score >= 80 ? "Conforme Ley 1581" : score >= 50 ? "Brechas prioritarias" : "Vulnerabilidades críticas"}
                </p>
              </div>
              <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-4">
                <p className="text-xs text-slate-400 font-semibold uppercase tracking-wide mb-1">Controles</p>
                <p className="text-xl font-extrabold text-cavaltec-dark">
                  {tasksDone}<span className="text-slate-300 font-normal text-base"> / {tasksTotal}</span>
                </p>
                ...
                <p className="text-xs text-slate-400 mt-1">Mitigados</p>
              </div>
            </div>
          </div>
